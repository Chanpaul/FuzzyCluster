function result = EFCMInitialv2(dist,dc)
...
%**********************find real connection between clusters********************
[trho,ordIndex]=sort(clustDomFeat);
sizeOfClustDom=size(clustDom,2);
borderRho=zeros(1,sizeOfClustDom);
dcl=zeros(1,ND);
for jj=1:sizeOfClustDom,
    x=rho(clustDom(jj).ele);
    [tmu,tsigma] = normfit(x);
    borderRho(jj)=tmu-0.5*tsigma; 
    dcl(clustDom(jj).ele)=jj;
%     d=pdf('norm',x,tmu,tsigma);
%     figure(jj);
%     plot(x,d,'.');
end;
...
for jj=1:sizeOfClustDom,
    ...
    for ii=1:tsz,
        ...
        for ss=1:size(dp(tid).dcNeigh,2);
            ...
            if isempty(isCon), 
                cp=dp(tid).dcNeigh(ss);
                if rho(tid)>borderRho(jj) && rho(cp)>borderRho(dcl(cp)),
